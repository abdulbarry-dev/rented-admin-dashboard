...
const selectedStatus = ref<'all' | 'completed' | 'pending' | 'cancelled' | 'refunded'>('all')
const selectedOrderStatus = ref<'all' | 'pending' | 'confirmed' | 'processing' | 'shipped' | 'delivered' | 'cancelled'>('all')
const sortBy = ref<'newest' | 'oldest' | 'amount-high' | 'amount-low'>('newest')

// Mock purchase transaction data
const transactions = ref<PurchaseTransaction[]>([
  ...
  },
  {
    ...
    buyer: { id: 19, name: 'Alex Johnson', email: 'alex@example.com' },
...
const filteredTransactions = computed(() => {
  ...
  if (searchQuery.value) {
    ...
    result = result.filter(
      t =>
        ...
        t.shipping?.trackingNumber.toLowerCase().includes(search)
    )
  }
  ...
  if (selectedOrderStatus.value !== 'all') {
    result = result.filter(t => t.orderStatus === selectedOrderStatus.value)
  }
...
const stats = computed(() => ({
  ...
  pendingFulfillment: transactions.value.filter(t => t.orderStatus === 'pending' || t.orderStatus === 'confirmed' || t.orderStatus === 'processing').length
}))
...
watch([searchQuery, selectedStatus, selectedOrderStatus, sortBy], () => {
  currentPage.value = 1
})
...
const handleUpdateShipping = (transactionId: number) => {
  router.push(`/transactions/shipping?transaction=${transactionId}`)
}

const handleExport = () => {
  console.log('Exporting purchase transactions...')
}
  ...
  <div class="min-h-screen bg-slate-50 dark:bg-slate-900 p-6">
      ...
      <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          ...
          <p class="text-slate-600 dark:text-slate-400">
            Manage purchase transactions and order fulfillment
          </p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-8">
      ...
      <ItemStatsCard
        ...
        :value="stats.pendingFulfillment.toLocaleString()"
        :icon="ClockIcon"
        color="orange"
      />
      <ItemStatsCard
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4 md:p-6 mb-6">
      ...
      <div class="mb-4">
        <div class="relative">
          ...
          <input
            v-model="searchQuery"
            type="text"
            placeholder="Search by transaction ID, item, buyer, seller, or tracking number..."
      ...
      <div class="mt-4 text-sm text-slate-600 dark:text-slate-400">
        Showing {{ filteredTransactions.length }} of {{ transactions.length }} purchase transactions
      </div>
    </div>
    ...
    <div v-else-if="paginatedTransactions.length > 0" class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
        ...
        <table class="w-full">
          ...
          <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
            ...
            >
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <BaseBadge :variant="getOrderStatusVariant(transaction.orderStatus)" size="sm">
                  {{ transaction.orderStatus.charAt(0).toUpperCase() + transaction.orderStatus.slice(1) }}
                </BaseBadge>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <div v-if="transaction.shipping" class="text-sm text-slate-600 dark:text-slate-400">
                  <div>{{ transaction.shipping.carrier }}</div>
                  <div class="text-xs font-mono">{{ transaction.shipping.trackingNumber }}</div>
                </div>
                <span v-else class="text-sm text-slate-400">No tracking</span>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex gap-2">
                  ...
                  <BaseButton
                    v-if="transaction.orderStatus !== 'delivered' && transaction.orderStatus !== 'cancelled'"
                    variant="outline"
                    size="xs"
                    @click="handleUpdateShipping(transaction.id)"
                  >
                    Ship
                  </BaseButton>
                </div>
      ...
      <div class="lg:hidden divide-y divide-slate-200 dark:divide-slate-700">
        ...
        >
          ...
          <div v-if="transaction.shipping" class="mb-3 p-2 bg-slate-50 dark:bg-slate-900/50 rounded-lg">
            <div class="text-xs text-slate-500 dark:text-slate-400 mb-1">Tracking</div>
            <div class="text-sm text-slate-900 dark:text-white">{{ transaction.shipping.carrier }}</div>
            <div class="text-xs font-mono text-slate-600 dark:text-slate-400">{{ transaction.shipping.trackingNumber }}</div>
          </div>
    ...
    <EmptyState
      ...
      description="No purchase transactions match your current filters. Try adjusting your search criteria."
    />
  </div>
</template>
